...
- **Per-page backdrop config via body data attributes:**
  ...
  - Current page settings: home (`/`, the former workshop file) `3`, about `12`, consulting `1`, links `0` + `ship="off"`. The `/workshop/` redirect stub has no backdrop.
...
└── shared/
    ...
    ├── aurora-live.js     asteroid + ship draw loop (ship explodes + field resets on collision), reads body data attrs, reduced-motion guard
