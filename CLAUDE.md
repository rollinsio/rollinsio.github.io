...
The single GitHub Pages site for `rollins.io`. The repo is `rollinsio/rollinsio.github.io` — a user-pages repo, so Pages serves its default branch (`main`) root. It hosts the landing page at `/` and microsites at `/<slug>/`. Today there are three content pages, two hidden archived pages, and one redirect stub:

- `/` — `index.html` (Michael Rollins · personal bio + the "Elsewhere" directory + the contact form — this *is* the home/landing page)
- `/architecture-audit/` — `architecture-audit/index.html` (**The Architecture Audit** — a paid five-part system review sold through Whop; the site's current primary offer). Time-boxed founding-group landing page: five analyses (Security / Scalability / Developer Environment / Deployment / General Architecture) + "the dragon in the box", two founding bonuses, a guarantee, $999, 20 spots, doors close Wed Jul 15. Single CTA links out to the Whop checkout (`https://whop.com/rollins-io-7c87/the-architecture-audit/`). Linked from the home hero card, the `/links/` featured card, and the top nav (`audit`).
...
- **Per-page backdrop config via body data attributes:**
  ...
  - Current page settings: home (`/`, the bio) `3`, architecture-audit `2`, links `0` + `ship="off"`. The hidden `/consulting/` archive keeps `1` and `/workshop/` keeps `3`. (Cap: no page should exceed `3` asteroids.) Live pages don't set `data-aurora-mode`, so all default to asteroids (only the `/spicy/` gag uses `missile`). The `/about/` redirect stub has no backdrop.
...
- **Identity header.** `.avatar.sm` (80px conic-gradient ring) anchored at the same Y across every page: `<main>` uses `py-12 md:py-16`, `<nav>` has `mb-10`, and the avatar sits at the top of a `text-center` `<header>` with `mx-auto mb-3`. Don't drift these values per page — they're what keeps the avatar from jumping when you click between pages. The base `.avatar` (130px) defined in `base.css` is unused on the live pages, kept only for the design-handoff exploration files. Container width and body layout differ by page purpose:
  - **`/` (home — the bio)** — editorial / expansive at `max-w-prose` (38rem). Identity header using the **home convention** (non-link avatar `<div>` + plain `rollins.io` eyebrow). Centered hero with two-line h1 (line 1 plain `--fg`, line 2 surname is `.aurora-text` clipped to the `accent → accent2` gradient — visibly chromatic under the spectrum wave), a `.bullet` list, one stacked `.featured.lg` callout (The Architecture Audit → `/architecture-audit/`), the `.row-link` **"Elsewhere"** directory (its `Contact` row jumps to `#interest` on the same page), then the contact form (`#interest`, see below) wrapped in a `.card`. The page-only CSS is the `.interest-form` field styles (`.aurora-text` is now shared in `base.css`).
  - **`/architecture-audit/`** — content-dense at `max-w-3xl`. Subpage convention: linked avatar back to `/` + `architecture audit · rollins.io` eyebrow. Hero (two-line h1, `.aurora-text` on "Audit.", founding-group eyebrow, Whop CTA + `#included` ghost), the five analyses as `.step` 01–05 (`#included`), a `.featured.lg` "dragon in the box" callout, two founding-bonus `.card`s, a guarantee `.card`, a single `.tier.featured > .inner` pricing card with the `$999` price + Whop CTA (`#claim`), FAQ. **No page-specific CSS** (uses shared vocabulary only). See the Architecture Audit sync points below.
  - **`/links/`** — share-targeted at `max-w-sm` (24rem). Centered identity, stacked `.featured` **The Architecture Audit** (→ `/architecture-audit/`, top) + **Beyond Test Coverage** (→ the GitHub repo) tiles, then a stack of `.link-btn` rows (its `Contact` row → `/#interest`). Optimized for being pasted into social bios.
...
├── architecture-audit/index.html  The Architecture Audit — paid five-part review, Whop checkout CTA (no page-specific CSS); site's primary offer
...
- Cross-page links use absolute paths (`/`, `/architecture-audit/`, `/links/`) so they work whether served from `rollins.io` root or a local `python3 -m http.server`. Do **not** link to `/about/` (redirect stub), `/consulting/` (hidden archive), or `/workshop/` (hidden archive) from any live page.
- Nav order is `home · audit · links` across every page (`home` → `/`, `audit` → `/architecture-audit/`). The current page is rendered as a plain `text-fg` span, others as `link` anchors. "about" was dropped from the nav when the bio became the home page; "consulting" was dropped when the Architecture Audit replaced it as the primary offer.
...
4. Open with the shared top-nav and centered identity header so the brand reads consistently:
   ...
   <nav class="text-center mb-10 fade-in">
       <div class="eyebrow flex justify-center items-center gap-3 flex-wrap">
           ...
           <a href="/architecture-audit/" class="link">audit</a>
           <span class="text-dim">·</span>
...
Then open `http://localhost:8000/` (the bio/home), `/architecture-audit/`, `/links/`. `/about/` should bounce you to `/`; `/consulting/` and `/workshop/` are hidden archives (reachable only by typing the URL).
...
- Sections: identity header (home convention), Hero (two-line h1 + `.bullet` list), one `.featured.lg` Architecture Audit callout (→ `/architecture-audit/`), the `.row-link` **"Elsewhere"** directory, then the contact form (`#interest`, see below) wrapped in a `.card`.
...
## Architecture Audit page — content sync points

When editing `architecture-audit/index.html` (the site's current primary offer):
...
- **Time-boxed offer — sync points that WILL go stale:** the **price** (`$999`, shown in the hero terms line, the founding-group eyebrows across home/links cards, and the `.tier` `.price`), the **spot count** (`20 spots`, in the eyebrows + `.tier-eyebrow`), and the **deadline** (`Wednesday, July 15 · 5 PM ET`, in the `#claim` terms line). If the offer changes or closes, update all three here **and** the two inbound featured cards (home hero + `/links/`).
- **Both CTAs link out to Whop** (`https://whop.com/rollins-io-7c87/the-architecture-audit/`, `rel="noopener"`) — the audit does not use the home contact form. If the Whop URL changes, update both CTAs on this page plus the two featured cards (which link to `/architecture-audit/`, not directly to Whop).
