...
- `/links/` — `links/index.html` (linktree-style stacked-button page for sharing in social bios; its hero tile is **MCP is Dead** → a claude.ai artifact)
...
- **Per-page backdrop config via body data attributes:**
  ...
  - Current page settings: home (`/`, the bio) `3`, links `0` + `ship="off"`. The hidden archives keep `/architecture-audit/` `2`, `/consulting/` `1`, `/workshop/` `3`. (Cap: no page should exceed `3` asteroids.) Live pages don't set `data-aurora-mode`, so all default to asteroids (only the `/spicy/` gag uses `missile`). The `/about/` redirect stub has no backdrop.
...
- **Identity header.** `.avatar.sm` (80px conic-gradient ring) anchored at the same Y across every page: `<main>` uses `py-12 md:py-16`, `<nav>` has `mb-10`, and the avatar sits at the top of a `text-center` `<header>` with `mx-auto mb-3`. Don't drift these values per page — they're what keeps the avatar from jumping when you click between pages. The base `.avatar` (130px) defined in `base.css` is unused on the live pages, kept only for the design-handoff exploration files. Container width and body layout differ by page purpose:
  ...
  - **`/links/`** — share-targeted at `max-w-sm` (24rem). Centered identity, one `.featured` hero tile — **MCP is Dead** (→ the *The MCP Surface Crossover* Claude artifact) — then a stack of `.link-btn` rows led by **Harness Reference** (→ the *Rex Harness Reference* artifact) and **Beyond Test Coverage** (→ the GitHub repo), then the social rows (its `Contact` row → `/#interest`). Optimized for being pasted into social bios. **Both artifact links are claude.ai artifacts shared "anyone with the link"** — if that sharing is ever revoked the public links break, and viewers see the pinned share version, not the latest publish.
