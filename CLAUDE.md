...
The single GitHub Pages site for `rollins.io`. The repo is `rollinsio/rollinsio.github.io` — a user-pages repo, so Pages serves its default branch (`main`) root. It hosts the landing page at `/` and microsites at `/<slug>/`. Today there are three live content pages, three hidden archived pages, one gag page, and one redirect stub:

- `/` — `index.html` (Michael Rollins · personal bio + the "Elsewhere" directory + the contact form — this *is* the home/landing page)
- `/architecture-audit/` — `architecture-audit/index.html` is the **archived Architecture Audit page**, deliberately hidden as of 2026-08-24: `<meta name="robots" content="noindex">` and **nothing on the live site links to it**. Reachable only by typing the exact `/architecture-audit/` URL. Kept intact so the offer can be relaunched; its two CTAs still point at the Whop checkout (`https://whop.com/rollins-io-7c87/the-architecture-audit/`), which stays live until deactivated in Whop. Time-boxed founding-group landing page: five analyses (Security / Scalability / Developer Environment / Deployment / General Architecture) + "the dragon in the box", two founding bonuses, a guarantee, $999, 20 spots, doors close Wed Jul 15.
- `/links/` — `links/index.html` (linktree-style stacked-button page for sharing in social bios; its hero tile is **MCP is Dead** → `/mcp-is-dead/`)
- `/mcp-is-dead/` — `mcp-is-dead/index.html` (write-up landing for the MCP-vs-Agent-SDK token benchmark: the argument in prose, then four outbound `.link-btn` rows — Experiment Write Up + Harness Reference (claude.ai artifacts), OpsDesk MCP (Glama) + OpsDesk GitHub. Linked only from the `/links/` hero tile; not in the nav or the home "Elsewhere" directory)
- `/consulting/` — `consulting/index.html` is the **archived engineering-leadership working-sessions page**, deliberately hidden: `<meta name="robots" content="noindex">` and **nothing on the live site links to it**. Reachable only by typing the exact `/consulting/` URL. Kept intact so it can be relaunched; its Inquire CTAs still point at `/?for=consulting-*#interest` (the home form still recognises those contexts). Superseded as the primary offer by the Architecture Audit.
...
- **Per-page backdrop config via body data attributes:**
  ...
  - Current page settings: home (`/`, the bio) `3`, links `0` + `ship="off"`, `/mcp-is-dead/` `2`. The hidden archives keep `/architecture-audit/` `2`, `/consulting/` `1`, `/workshop/` `3`. (Cap: no page should exceed `3` asteroids.) Live pages don't set `data-aurora-mode`, so all default to asteroids (only the `/spicy/` gag uses `missile`). The `/about/` redirect stub has no backdrop.
...
- **Identity header.** `.avatar.sm` (80px conic-gradient ring) anchored at the same Y across every page: `<main>` uses `py-12 md:py-16`, `<nav>` has `mb-10`, and the avatar sits at the top of a `text-center` `<header>` with `mx-auto mb-3`. Don't drift these values per page — they're what keeps the avatar from jumping when you click between pages. The base `.avatar` (130px) defined in `base.css` is unused on the live pages, kept only for the design-handoff exploration files. Container width and body layout differ by page purpose:
  - **`/` (home — the bio)** — editorial / expansive at `max-w-prose` (38rem). Identity header using the **home convention** (non-link avatar `<div>` + plain `rollins.io` eyebrow). Centered hero with two-line h1 (line 1 plain `--fg`, line 2 surname is `.aurora-text` clipped to the `accent → accent2` gradient — visibly chromatic under the spectrum wave), a `.bullet` list, the `.row-link` **"Elsewhere"** directory (its `Contact` row jumps to `#interest` on the same page), then the contact form (`#interest`, see below) wrapped in a `.card`. The page-only CSS is the `.interest-form` field styles (`.aurora-text` is now shared in `base.css`).
  - **`/architecture-audit/` (hidden archive)** — content-dense at `max-w-3xl`, `noindex`, unlinked. Subpage convention: linked avatar back to `/` + `architecture audit · rollins.io` eyebrow. Hero (two-line h1, `.aurora-text` on "Audit.", founding-group eyebrow, Whop CTA + `#included` ghost), the five analyses as `.step` 01–05 (`#included`), a `.featured.lg` "dragon in the box" callout, two founding-bonus `.card`s, a guarantee `.card`, a single `.tier.featured > .inner` pricing card with the `$999` price + Whop CTA (`#claim`), FAQ. **No page-specific CSS** (uses shared vocabulary only). See the Architecture Audit sync points below.
  - **`/links/`** — share-targeted at `max-w-sm` (24rem). Centered identity, one `.featured` hero tile — **MCP is Dead** (→ `/mcp-is-dead/`, the on-site write-up) — then a stack of `.link-btn` rows led by **Harness Reference** (→ the *Rex Harness Reference* artifact) and **Beyond Test Coverage** (→ the GitHub repo), then the social rows (its `Contact` row → `/#interest`). Optimized for being pasted into social bios. **The claude.ai artifact links (here and on `/mcp-is-dead/`) are shared "anyone with the link"** — if that sharing is ever revoked the public links break, and viewers see the pinned share version, not the latest publish.
  - **`/mcp-is-dead/`** — editorial at `max-w-prose` (38rem). Subpage convention: linked avatar back to `/` + `mcp is dead · rollins.io` eyebrow. Nav is the plain `home · links` pair (this page is not a nav item). Hero (two-line h1, `.aurora-text` on "Dead.", `Benchmark` eyebrow) followed by the argument as five paragraphs, then a four-row `.link-btn` stack and a `← back to links` `.btn-ghost`. **No page-specific CSS.**
  - **`/consulting/` (hidden archive)** — content-dense at `max-w-3xl`, `noindex`, unlinked. Subpage convention: linked avatar back to `/` + `consulting · rollins.io` eyebrow. Hero, "The shift" (3-card grid), Approach (steps 01–05 day-1 + day-divider + steps 06–07 day-2), Outcomes (4-card grid), Engagements (two `.tier` cards — Two-day uses `.tier.featured > .inner` for Recommended emphasis), Logistics `.kv` card, FAQ. Its Inquire CTAs still route to `/?for=consulting-*#interest`.
...
├── index.html                  home / landing = personal bio + "Elsewhere" directory + contact form (page-only CSS: .interest-form fields)
├── architecture-audit/index.html  archived Architecture Audit — hidden (noindex, unlinked); kept for relaunch (no page-specific CSS)
...
├── consulting/index.html       archived working-sessions page — hidden (noindex, unlinked); kept for relaunch (page-only CSS: .day-divider)
├── links/index.html            share-targeted link landing (no page-specific CSS)
├── mcp-is-dead/index.html      MCP-vs-Agent-SDK benchmark write-up + outbound links; linked from the /links/ hero (no page-specific CSS)
├── spicy/index.html            gag page ("Why did you click?") — noindex; linked only from /links/ (no page-specific CSS)
...
└── shared/
    ...
    ├── base.css           .eyebrow, .link, .row-link, .bullet, .aurora-text, .card, .avatar (+ .sm modifier), .fade-in, reduced-motion guard
...
- Cross-page links use absolute paths (`/`, `/links/`) so they work whether served from `rollins.io` root or a local `python3 -m http.server`. Do **not** link to `/about/` (redirect stub), `/architecture-audit/`, `/consulting/`, or `/workshop/` (all hidden archives) from any live page.
- Nav order is `home · links` across every page. The current page is rendered as a plain `text-fg` span, others as `link` anchors. "about" was dropped from the nav when the bio became the home page; "consulting" when the Architecture Audit replaced it as the primary offer; "audit" when the audit itself was archived (2026-08-24).
...
Then open `http://localhost:8000/` (the bio/home) and `/links/`. `/about/` should bounce you to `/`; `/architecture-audit/`, `/consulting/`, and `/workshop/` are hidden archives (reachable only by typing the URL).
...
- Sections: identity header (home convention), Hero (two-line h1 + `.bullet` list), the `.row-link` **"Elsewhere"** directory, then the contact form (`#interest`, see below) wrapped in a `.card`.
- This page is intentionally **evergreen** — no dates, times, or durations to keep in sync.
- The **"Elsewhere"** directory is the only place that link directory lives. Its `Contact` row jumps to `#interest` (the on-page form). Keep its rows aligned with the `/links/` stack (GitHub, YouTube, Will AI Snap That, Instagram, LinkedIn, Contact). The `/links/`-only content links (MCP is Dead, Harness Reference, Beyond Test Coverage) are deliberately not mirrored here.
...
**This is the site's single contact funnel** — it moved here from the (now archived) workshop page. The home's own `Elsewhere → Contact` row jumps to `#interest`, and the `/links/` `Contact` row links to `/#interest`; with no `for` param the form shows generic "Get in touch" copy (default `_subject` = `Contact — rollins.io`). The now-archived consulting page's two **Inquire** CTAs still link cross-page to `/?for=consulting-one-day#interest` / `/?for=consulting-two-day#interest`; the inline script reads the `for` query param and rewrites the hidden `_subject` plus the form's eyebrow/heading/blurb (ids `interest-subject` / `interest-eyebrow` / `interest-heading` / `interest-blurb`), then smooth-scrolls to `#interest`. Recognised contexts: `consulting-one-day`, `consulting-two-day` (kept so the archived page still works — no live page uses them). The Architecture Audit does **not** funnel here; it sends buyers straight to Whop. The only `mailto:` left anywhere on the site is the `#interest-error` failure fallback.

- Form CSS lives in a small `<style>` block on the home page (`index.html`) only (`.interest-form .field`, `.lbl`, inputs, `.hp` honeypot positioning). It's the only page-specific CSS this page owns (`.aurora-text`, previously here, is now shared in `base.css`).
...
## Architecture Audit page — content sync points (archived)

`/architecture-audit/` is a hidden archive (`noindex`, unlinked) as of 2026-08-24 — the user pulled it from every page. Kept intact for relaunch. When editing `architecture-audit/index.html`:

- Sections: identity header (subpage convention), Hero (two-line h1 with `.aurora-text` on "Audit.", founding-group eyebrow, Whop CTA + `#included` ghost, a mono terms line), the five analyses as `.step` 01–05 (`#included`), a `.featured.lg` "dragon in the box" callout, two founding-bonus `.card`s, a guarantee `.card`, a single `.tier.featured > .inner` pricing card (`#claim`) with the `$999` price + Whop CTA, then FAQ.
- **The five analyses are fixed and ordered:** 01 Security · 02 Scalability · 03 Developer Environment · 04 Deployment · 05 General Architecture — "one professional recommendation for each". Keep this order and count aligned across the `.step` list, the hero/pricing blurbs, and the home + `/links/` featured-card `meta` copy.
- **Time-boxed offer — sync points that WILL go stale:** the **price** (`$999`, shown in the hero terms line, the founding-group eyebrows across home/links cards, and the `.tier` `.price`), the **spot count** (`20 spots`, in the eyebrows + `.tier-eyebrow`), and the **deadline** (`Wednesday, July 15 · 5 PM ET`, in the `#claim` terms line). If the offer is relaunched, update all three here **and** re-add the two inbound featured cards (home hero + `/links/`).
- **Both CTAs link out to Whop** (`https://whop.com/rollins-io-7c87/the-architecture-audit/`, `rel="noopener"`) — the audit does not use the home contact form. **Still live** — deactivate the product in Whop if you want to stop stray purchases while the page is hidden. To relaunch: restore inbound links (an `audit` nav item, a home `.featured.lg` card, a `/links/` featured card at the top of the stack), remove the `noindex`, and refresh the price / spot count / deadline below.
- Bonuses ($499 Loom video, $199 CI/CD checklist) and the guarantee are copy-only — no external links.

## Consulting page — content sync points (archived)

`/consulting/` is a hidden archive (`noindex`, unlinked) as of 2026-07-13 — the Architecture Audit replaced it as the primary offer. Kept intact for relaunch. When editing `consulting/index.html`:
...
- To relaunch: restore inbound links (nav item, a home `.featured.lg` card, a `/links/` featured card), remove the `noindex`, and re-verify the Inquire CTAs still resolve.
...
- **2026-07-13** — **New primary offer: The Architecture Audit; consulting archived.** (1) Built `architecture-audit/index.html` — a time-boxed founding-group landing page for a paid five-part system review sold via Whop (`https://whop.com/rollins-io-7c87/the-architecture-audit/`). `max-w-3xl`, subpage identity convention, `data-aurora-asteroids="2"`. Sections: hero (`.aurora-text` on "Audit."), five analyses as `.step` 01–05 (Security / Scalability / Developer Environment / Deployment / General Architecture), a `.featured.lg` "dragon in the box" callout, two founding-bonus `.card`s ($499 Loom + $199 CI/CD checklist), a guarantee `.card`, a `.tier.featured > .inner` `$999` pricing card, FAQ. Both CTAs link out to Whop; no page-specific CSS. (2) **Pivoted the primary offer:** home hero `.featured.lg` swapped Consulting → Architecture Audit; `/links/` swapped the Consulting featured card → Architecture Audit (now top, above Beyond Test Coverage); top nav re-labelled `home · consulting · links` → **`home · audit · links`** across home/links/consulting + the new page. (3) **Archived `/consulting/`** (per the user, mirroring `/workshop/`): added `noindex`, removed all inbound links, fixed its nav to the new live set — file kept intact for relaunch; its Inquire CTAs + the home form's `consulting-*` CTX left working. (4) **Consolidated `.aurora-text`** (was duplicated page-local in `index.html` + `spicy/index.html`, now needed a 3rd copy) into `shared/base.css`; removed both page-local copies (spicy's `<style>` block is now gone entirely). (5) CLAUDE.md updated throughout (page inventory, per-page backdrop settings, layout tree, editing rules, nav snippet, preview list, Express-interest note, new Architecture Audit sync-points section, consulting section marked archived). Verified locally (python http.server + full-page screenshots of `/architecture-audit/`, `/`, `/links/` — all render correctly, `.tier.featured > .inner` intact). **Not committed** — left in the working tree pending the user's explicit go-ahead (no-commit-on-`main` rule).
- **2026-08-24** — **Architecture Audit pulled from the live site.** Per the user, removed the `/links/` top featured card, the home `.featured.lg` hero card, and the `audit` nav item everywhere (nav is now `home · links` on `/`, `/links/`, and the archived `/consulting/`). `architecture-audit/index.html` kept intact and archived like `/consulting/` + `/workshop/`: added `<meta name="robots" content="noindex">`, zero inbound links, reachable only by the exact URL. Its Whop CTAs are untouched and **still live** — deactivate the product in Whop to stop stray purchases while hidden. `/links/` Beyond Test Coverage card is now the top tile (fade delay 0.07 → 0.05). CLAUDE.md updated throughout.
- **2026-08-24 (cont.)** — `/links/` hero swapped to **MCP is Dead** (`.featured` → `https://claude.ai/code/artifact/bd0bfa94-2b44-4dcc-863e-32bc8b1f5a43`, the *MCP Surface Crossover* benchmark artifact; copy pulled from the artifact's own headline — 48 tools vs six, 2.37× cheaper via SDK, 144 runs / 4 arms). **Beyond Test Coverage demoted** from a featured tile to a plain `.link-btn`, and a new **Harness Reference** `.link-btn` added above it (→ `https://claude.ai/code/artifact/2ee2991d-4f62-4a6c-9cd8-f3184dd351e5`). Stack fade delays re-staggered 0.10 → 0.34. Both artifacts are shared "anyone with the link"; viewers see the pinned share version. `links/index.html` + this doc only; the home page was not touched.
- **2026-08-25** — Added **`/mcp-is-dead/`** (`mcp-is-dead/index.html`), a real on-site page for the MCP-vs-Agent-SDK token benchmark: subpage identity convention, `max-w-prose`, `data-aurora-asteroids="2"`, hero (`.aurora-text` on "Dead.") + the user's argument verbatim in five paragraphs, then four outbound `.link-btn` rows (Experiment Write Up → the `bd0bfa94…` artifact, Harness Reference → the `2ee2991d…` artifact, OpsDesk MCP → Glama, OpsDesk GitHub → `katareayush/opdesk-mcp`) and a `← back to links` ghost button. No page-specific CSS. **`/links/` hero re-pointed** from the artifact URL to `/mcp-is-dead/` (dropped the now-unneeded `rel="noopener"`); its Harness Reference row left as-is. Deliberately kept out of the top nav and the home "Elsewhere" directory. Verified structurally (copy byte-matches the source text, all four URLs present, no `.featured`-without-`.inner`); the Chrome extension was offline so no screenshot pass.
